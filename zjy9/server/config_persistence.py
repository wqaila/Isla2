...
# 默认配置（首次运行或文件不存在时使用）
# ⚠️ 这里的默认值必须与 config.py 里的常量保持一致，否则会出现
#    "没改过配置，但实际生效值和 config.py 写的不一样" 的诡异现象。
_DEFAULT_CONFIG = {
    ...
    # Ollama 上下文窗口。必须能装下：人设 + Few-Shot + 记忆 + 画像 + 历史，
    # 调小会导致 System Prompt 被静默截断（人设崩坏）。
    "num_ctx": 4096,
    # 是否注入 Few-Shot 示例（关掉可显著节省上下文，长对话场景建议关）
