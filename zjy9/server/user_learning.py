...
class UserLearningEngine:
    ...
    def _init_patterns(self):
        ...
        self.personal_patterns = {
            ...
            "gender": [
                # ⚠️ 必须带捕获组：_extract_personal_info 在没有捕获组时会退回
                #    match.group(0)，也就是把"我是男生"整句存进 profile.gender。
                r"我是\s*(男生|男孩|男|女孩子|女孩|女|男人|女人)",
                r"我(?:是|为)\s*(男性|女性)",
            ],
            "location": [
    ...
    def get_personalized_context(self, current_message: str = "", max_chars: int = 1000,
                                 device_id: str = "") -> str:
        """获取个性化上下文（按设备隔离用户画像）"""
        with self.device_scope(device_id):
            return self._get_personalized_context_impl(current_message, max_chars)

    def _get_personalized_context_impl(self, current_message: str = "", max_chars: int = 1000) -> str:
        """
...
- openness: 高分=聊新事物、创意话题；低分=保守、重复话题"""
    ...
    def apply_llm_analysis(self, analysis: dict, device_id: str = ""):
        """
        应用 LLM 分析结果到用户画像（按设备隔离）

        Args:
            analysis: LLM 返回的分析结果字典
            device_id: 目标设备，空则沿用当前设备
        """
        with self.device_scope(device_id):
            return self._apply_llm_analysis_impl(analysis)

    def _apply_llm_analysis_impl(self, analysis: dict):
        """应用 LLM 分析结果（内部实现，调用前需已切换到目标设备）"""
        # 1. 更新性格特征（与现有值做加权平均）
    ...
    def __init__(self):
        ...
        # 对话缓冲区（用于 LLM 分析）
        # 按设备隔离：之前是全局单列表，多设备场景下会把不同用户的对话
        # 混在一起送去做 LLM 分析，导致画像串号。
        self._message_buffers: dict[str, list] = {}
        self._buffer_size = 10  # 每 10 条消息触发一次 LLM 分析
        self._llm_pending: bool = False  # 是否有待处理的 LLM 分析请求

        # 设备切换锁：use_device() 会改动 self.profile，
        # 必须串行化，否则并发请求会把用户 A 的消息记到用户 B 头上。
        self._device_lock = threading.RLock()

    @property
    def _message_buffer(self) -> list:
        """当前设备的消息缓冲区"""
        return self._message_buffers.setdefault(self._current_device, [])

    def use_device(self, device_id: str) -> None:
        """把引擎切换到指定设备的画像（线程安全）。

        这是多用户支持真正被接上的入口：之前 set_device() 定义了却全项目零调用，
        导致所有设备共用同一个 profile。
        为避免每条消息都写盘，这里只在内存缓存里切换，落盘交给 save_profile 的周期逻辑。
        """
    ...
    @contextmanager
    def device_scope(self, device_id: str):
        """在指定设备的画像上下文内执行一段逻辑"""
        with self._device_lock:
            self.use_device(device_id)
            yield
...
- 简洁明了，不超过 100 字"""
    ...
    def add_to_buffer(self, role: str, content: str):
        ...
        # 保持缓冲区大小（原地截断，_message_buffer 是只读属性不能整体赋值）
        if len(buf) > 30:
            del buf[:-20]
    ...
    def _apply_correction(self, correction: str):
        ...
        # 收紧说明：原模式 `不是(.{1,20})，是(.{1,20})` 会把
        # "不是我不想，是真的没时间" 判成"把 我不想 改成 真的没时间"，
        # 从而篡改知识图谱。这里统一要求第一人称 + 更短的内容片段。
        correction_patterns = [
            r"我(?:说的)?不是([^，,。！？]{1,10})[，,]\s*(?:而)?是([^，,。！？]{1,10})",
            r"我[^，,。！？]{0,8}不对[，,]\s*(?:应该|其实)是([^，,。！？]{1,10})",
            r"(?:更正|纠正|订正|修正)[：:]?\s*([^，,。！？]{1,10})\s*(?:→|->|是)\s*([^，,。！？]{1,10})",
        ]

        for pattern in correction_patterns:
            match = re.search(pattern, correction)
            if not match or (match.lastindex or 0) < 2:
                continue
            wrong = (match.group(1) or "").strip()
            right = (match.group(2) or "").strip()

            # 过滤太短或无效的匹配（单字替换极易误伤其他节点内容）
            if len(wrong) < 2 or len(right) < 1:
                continue
    ...
    def detect_correction(self, user_message: str) -> bool:
        """
        检测用户消息是否包含"纠正之前说过的话"的意图

        早期版本用的是 `不是.{1,20}[，,].{1,20}` 这类极宽松的模式，
        结果"不是我不想，是真的没时间"这种日常表达也会被判成纠正，
        进而去改写知识图谱，属于"误伤用户记忆"。
        这里收紧为：必须出现**明确的纠正动词**，或者**第一人称 + 极短的否定-修正对**。
        """
        correction_patterns = [
            # 1) 明确的纠正动作
            r"(?:更正|纠正|订正|修正)[：:，,]?\s*\S",
            ...
            r"我说的不是",
            r"我(?:之前)?(?:说的|记)错了",
            # 2) "我(说的)不是A，是B"：限定第一人称 + A 很短（≤10字），
            #    避免把"不是我不想，是真的没时间"这类句子误判
            r"我(?:说的)?不是[^，,。！？]{1,10}[，,]\s*(?:而)?是",
            # 3) "X 不对，应该是 Y"，X 必须紧跟在第一人称之后
            r"我[^，,。！？]{0,8}不对[，,]\s*(?:应该|其实)是",
        ]
    ...
    def learn_from_message(self, user_message: str, ai_response: str = "",
        ...
        device_id 为空时沿用当前设备（兼容旧调用）。
        """
        with self.device_scope(device_id):
            return self._learn_from_message_impl(user_message, ai_response, session_id)

    def _learn_from_message_impl(self, user_message: str, ai_response: str = "", session_id: str = ""):
        """
        从一条对话中学习（增强版 v2）
