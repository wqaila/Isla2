...
# ===== CORS 配置 =====
# 默认只允许本机来源。需要从局域网/公网页面访问时，在此追加具体来源，
# 或设置 CORS_ORIGINS = ["*"]（不推荐，等于对所有网站开放）。
CORS_ORIGINS = [
    "http://localhost:8080",
    "http://127.0.0.1:8080",
    "http://localhost:5173",
...
def runtime(key: str, default=None):
    """读取运行时配置（持久化在 data/runtime_config.json），读不到则回退默认值。

    之前 config.py 里的常量在启动后就被固化，PUT /api/config 写进 JSON 也没人读，
    导致限流、超时、连接数等配置"改了不生效"。所有需要支持热更新的配置项
    都应通过本函数读取。

    这里用延迟导入，避免 config <-> config_persistence 的循环依赖。
    """
    try:
