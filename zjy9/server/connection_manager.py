...
class ConnectionManager:
    ...
    def get_stats(self) -> dict:
        ...
        ⚠️ main.py 的 /api/health/detailed 一直在调用本方法，
        但此前类里根本没有实现，该接口必定抛 AttributeError。
        """
        now = datetime.now()
        idle_seconds = []
        for info in self.active_connections.values():
            ...
            if not last:
                continue
            try:
                idle_seconds.append((now - datetime.fromisoformat(last)).total_seconds())
        ...
        return {
            ...
            "max": MAX_CONNECTIONS,
            "log_observers": len(self.log_observers),
            "idle_avg_seconds": round(sum(idle_seconds) / len(idle_seconds), 1) if idle_seconds else 0,
